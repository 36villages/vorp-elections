...
function OpenStartMenu(registered, city, region, onBallot)
    ...
    if onBallot then
        addMenuElement = {
            label = "Stop Running for Office",
            value = "stoprunning",
            desc =
            "Remove yourself from running for office"
        }
        table.insert(menuElements, 1, addMenuElement)
    else
        ...
        VORPMenu.Open(
            ...
            function(data, menu)
                ...
                elseif data.current.value == "exit_menu" then
                    ...
                    CastVote(registered, city, region, position, jurisdiction, selectedCandidateID, selectedBallotID,
                        onBallot)
                end
            end,
            function(data, menu)
...
function CastVote(registered, city, region, position, jurisdiction, candidateid, ballotid, onballot)
    ...
    TriggerEvent("vorp:ExecuteServerCallBack", "democracy:hasvotervotedalready", function(cb)
        ...
        if cb then
            ...
            TriggerEvent("vorpinputs:getInput", button, placeholder, function(answer)
                ...
                if answer == "y" or answer == "Y" then
                    ...
                    TriggerEvent("vorp:TipBottom",
                        ("You have cast your vote for " .. jurisdiction .. " " .. position .. " in " .. vcity .. " " .. vregion),
                        4000)
                    OpenVoteMenu(registered, city, region, onBallot)
                else
            ...
            local button = "You are about to place a vote for " ..
                jurisdiction .. " " .. position .. " in " .. vcity .. " " .. vregion .. " " .. " Press y to confirm."
            local placeholder = "y or n"
            TriggerEvent("vorpinputs:getInput", button, placeholder, function(answer)
                print("User input received:", answer)
                if answer == "y" or answer == "Y" then
